...
export default function ShowcaseDevice({ lang }: ShowcaseDeviceProps) {
  ...
  const rotateY = useTransform(scrollYProgress, [0, 1], [-7, 0]);
  const translateY = useTransform(scrollYProgress, [0, 1], [36, 0]);
  const scale = useTransform(scrollYProgress, [0, 1], [0.96, 1.02]);
  const opacity = useTransform(scrollYProgress, [0, 0.8], [0.62, 1]);
